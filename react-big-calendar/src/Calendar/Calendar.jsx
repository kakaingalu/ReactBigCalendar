...
import { selectCalendarEvents, setCalendarEvents } from '../redux/features/calendarEventSlice.jsx';
...
import axiosInstance from '../services/httpService.jsx';

const localizer = momentLocalizer(moment);

const getBGColor = (reason) => {
    switch (reason) {
...
const EventItem = ({ event }) => {
    function formatSidebarEventTime(startDate, endDate) {
        ...
        <div className="mb-2 shadow-sm calendar-event-item rounded-lg overflow-hidden">
            <div className="p-2">
                <div className="flex items-center mb-1">
                    ...
                    <span className="font-bold text-sm text-gray-500">{formatSidebarEventTime(event.start_date, event.end_date)}</span>
                </div>
...
const Calendar = () => {
    ...
    const [editEventInfo, setEditEventInfo] = useState(null);
    ...
    const handleEventAddSuccess = (newEventData) => {
        fetchEvents();
        hideModals();
    };

    const handleEventUpdateSuccess = (updatedEventData) => {
        fetchEvents();
        hideModals();
    };

    const deleteEventFromCalendar = (eventInfo) => {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                try {
                    ...
                    setEventsFromServer(prev => prev.filter(e => e.id.toString() !== eventId.toString()));
