...
import { selectEmployees, setEmployee } from '../../../redux/features/employeeSlice.jsx'; 
import { selectCases, setCase as setCasesRedux } from '../../../redux/features/casesSlice.jsx';
...
const createNewCalendarEventAPI = async (eventDetails, selectedInfo) => {
    ...
    try {
        ...
        throw error.response?.data || error; // Throw more specific error
    }
};


const NewEventModal = ({ selectedInfo, show, onHide, onEventAddSuccess }) => {
    const dispatch = useDispatch();
    const allCases = useSelector(selectCases); 
    const storedEmployees = useSelector(selectEmployees); 
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                // Fetch Employees (for MultipleChipSelect) if not in Redux
                if (!storedEmployees || storedEmployees.length === 0) {
                    try {
                        const employeeResponse = await axiosInstance.get('/pms/employees/'); 
                        
                        if (employeeResponse.data) {
                            dispatch(setEmployee(employeeResponse.data)); 
                        } else {
                            console.warn("No employee data received from API");
                        }
                    } catch (error) {
                        console.error('Error fetching employees with axiosInstance:', error);
                        // Optionally, show a user-facing error message or handle specific error statuses
                        // Swal.fire('Error', `Could not load employee data: ${error.message || 'Unknown error'}`, 'error');
                    }
                }
            } catch (error) {
    ...
    const handleCaseChange = (e, setFieldValue) => {
    ...
    const handleSubmit = async (values, { setSubmitting }) => {
        try {
            ...
            Swal.fire('Error!', `Fill All Fields Correctly. Error: ${error.message || 'Unknown error'}`, 'error');
        } finally {
            setSubmitting(false);
