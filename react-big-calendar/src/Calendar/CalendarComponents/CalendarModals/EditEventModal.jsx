...
// import { Modal, Button, Form as BootstrapForm, Row, Col } from 'react-bootstrap'; // Renamed Form to BootstrapForm
...
import { selectEmployees } from '../../../redux/features/employeeSlice';
import { useSelector } // useDispatch removed if not used directly here
    from 'react-redux';
...
// MUI Component - Kept as is for functionality
const MultipleChipSelect = ({ setFieldValue, initialAttendees = [] }) => {
    const storedEmployees = useSelector(selectEmployees);
    const theme = useTheme();
    const [personName, setPersonName] = useState([]);
    // const [attendeesSources, setAttendeesSources] = useState(storedEmployees); // Renamed for clarity

    // Initialize personName from initialAttendees (IDs)
    useEffect(() => {
        if (initialAttendees && Array.isArray(initialAttendees)) {
             // Assuming initialAttendees is an array of IDs
            setPersonName(initialAttendees);
        }
    }, [initialAttendees]);

    // Fetch employees if not available in Redux store (optional, if Redux isn't always populated)
    // useEffect(() => {
    // if (!storedEmployees || storedEmployees.length === 0) {
    //     const fetchEmployees = async () => { /* ... fetch logic ... */ setAttendeesSources(data); };
    //     fetchEmployees();
    // }
    // }, [storedEmployees]);
    ...
    const handleChange = (event) => {
        ...
        setFieldValue("attendees", newAttendees); // Send array of IDs to Formik
        setPersonName(newAttendees);
    };

    // Ensure storedEmployees is an array before mapping
    const employeeOptions = Array.isArray(storedEmployees) ? storedEmployees : [];
        ...
        <div>
            <FormControl sx={{ m: 1, width: '100%' }}>
                ...
                <Select
                    ...
                    id="attendees"
                    multiple
                    value={personName} // Should be an array of IDs
                ...
                >
                    {employeeOptions.map((employee) => (
                        <MenuItem
                            key={employee.id}
                            value={employee.id} // Value should be the ID
                            style={getStyles(employee.full_name, personName.map(id => { // Map IDs back to names for styling comparison if needed, or simplify getStyles
...
const EditEventModal = ({ eventInfo, show, onHide, onDeleteEvent, onEventUpdateSuccess }) => {
